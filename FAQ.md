...
:one:**仅编码器 Encoder-only**

​	核心特点：双向理解上下文，更擅长理解类任务

- **优点**

  - **理解能力强**：因为他能双向看上下文，所以比较适合文本的分类、情感分析、语义匹配和检索排序等任务
  - **适合做文本表示**：它可以把一句话编码成向量，用于相似度计算、RAG检索、聚类和推荐等
  - **训练目标适合理解：**BERT 常用 Masked Language Model

- **缺点❌**

  - **不擅长自然生成长文本：**BERT 不是按顺序一个词一个词生成的，所以不适合直接做聊天、写文章写代码等
  - **推理时不适合SSE**

  

:two:**仅解码器 Decoder-only**

​	核心特点：从左到右生成文本，更擅长生成类任务、对话

- **优点**

  - **生成能力强：**适合续写、问答、对话、代码生成和文章生成等任务
  - **适合SSE**
  - **架构简单统一：**同一个模型可以通过 prompt 适配很多任务：分类、摘要、翻译、代码、推理和工具调用等

- **缺点❌**

  - **做纯向量表示不一定最优：**如果你指向做句向量、语义检索，BERT/Embedding 模型通常更适合
  - **生成的内容不稳定：**会有幻觉、跑题、格式不稳定等问题，需要 prompt、约束解码、工具调用或后处理来控制
  - **长输入到端输出任务可能成本较高：**比如只想判断一句话情感，用GPT可以做，但通常比小型 BERT 分类模型更贵、更慢

  



:three:**编码器-解码器 Encoder-Decoder**

​	核心特点：将 **输入** 编码，在解码 **输出**，更擅长输入到输出的转换任务

- **优点**
  - **非常适合 Seq2Seq  的任务：**比如翻译、摘要、改写、问答、纠错和文本规范化等
  - **输入理解和输出生成分工明确**
  - **对输入输出长度差异大的任务友好**
- **缺点**
  - **结构复杂**
...
**Q2：** **自注意力机制如何使大模型能够捕获长距离依赖关系，它和RNN有什么区别？**

自注意机制的核心是：**序列中的每个 Token 都可以直接看见其他的 Token，并会根据相关性分配不同的权重**
...
```python
第 1 个词 -> 第 2 个词 -> 第 3 个词 -> ... -> 第 N 个词 
```

它需要把前面的信息压缩到隐藏状态里，一步步传下去，如果两个词距离很远，信息就会出现遗忘或者衰减的情况，而自注意力不会



**Q3：** **大模型为什么有上下文长度的概念？为什么它是指输入和输出的总长度？**

大模型一次只能处理有限数量的 token，这个上限就叫做 **上下文长度**

:a: **为什么会有上下文长度限制？**

- **计算量限制**

自注意力需要计算 token 和 token 之间的关系。假设输入有 1000 个 token，就要考虑很多 token 两两之间的关系。如果变成 10000 个token，计算量会大幅增加

- **显存限制**

上下文越长，模型需要保存的中间结果越多，占用显存也越大

- **位置编码的限制**

模型需要知道每个 token 在序列中的位置，不同的模型位置编码设计也不同，也会影响最大上下文长度



**:b: 为什么上下文长度是 “输入 + 输出” 的总长度**

因为模型生成回答时，并不是把输入和输出分开处理的，而是把它们放在同一个连续的序列里面
...
传统的中文分词的目标是**切出词**，而**大模型的分词是切出词元**。另外，对于指定的词表，不一定只有唯一的分词方式，它还受分词算法以及数据集的影响。

举个例子：对于 【我喜欢机器学习】这句话来说，传统的中文分词只有

```python
我 / 喜欢 / 机器 / 学习
```

这种拆分方式，**而大模型可能还会有如下的分词方法**
...
在调用大模型时，输入文本会加入特殊格式，也就是 chat template。例如
...
这里的 `<|user|>`、`<|assistant|>` 就是角色标记，不同的角色作用不同，如下表所示

 

| 角色      | 作用                             |
| --------- | -------------------------------- |
| system    | **规定模型行为、风格、安全边界** |
| user      | 用户提出的问题或任务             |
| assistant | AI 之前的回答                    |
| tool      | 工具返回的信息                   |
| developer | 开发者层面的约束或业务规则       |





**Q6：** **传统的静态词嵌入（如word2vec）与大模型产生的上下文相关的嵌入相比，有什么区别？有了上下文相关的嵌入，静态词嵌入还有什么价值？**

**静态词嵌入**是一个词一个固定的向量，而 **上下文相关的嵌入**是对于同一个词来说，可能在不同语境中向量是不同的。

**静态词嵌入的价值有：**

- 在一些传统的 NLP 模型中，可以用预训练的 Word2Vec 作为初始化向量
- 在小数据集场景中仍然实用
...
**Q7：** **在word2vec等词嵌入空间中，存在 `king - man + woman ≈ queen` 的现象，这是为什么？大模型的词元嵌入空间是否也有类似的属性？**

因为词向量在大量上下文中学习到了语义关系，某些关系在线性空间中表示为方向。

大模型有类似属性，且更复杂，有输入的 embedding_matrix 和经过 Transformer 多层计算后的上下文相关的 hidden states
